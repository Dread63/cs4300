...
|   |-- task4.py              # Duck-typed discount calculator
...
|-- tests/
|   |-- test_task1.py         # ... through test_task7.py, one per task
|-- conftest.py
|-- requirements.txt          # Recorded dependencies (pip freeze)
```
...
Each task is a standalone script. Run them individually from the project root:
...
From the project root, run the full suite with:
...
| 2 | `task2.py` | Demonstrates int, float, string, and boolean types |
| 3 | `task3.py` | Positive/negative/zero check, first 10 primes, sum 1–100 |
| 4 | `task4.py` | `calculate_discount` accepting any numeric type (duck typing) |
| 5 | `task5.py` | Book list with slicing; student-name → ID dictionary |
| 6 | `task6.py` | `read_file` counts words in `task6_read_me.txt` |
